...
router.get('/', async (req, res) => {
    ...
    const data = await product.select(params,
        ['-password'], {
        path: 'person',
        model: 'Persons'
    })
...
router.get('/:id', async (req, res) => {
    ...
    const data = await product.select({ _id: params.id },
        ['-password'],
        {
            path: 'person',
            model: 'Persons'
...
router.post('/', async (req, res) => {
    ...
    const ins = await product.insert(data)
    res.send(ins)
})
...
router.put('/:id', async (req, res) => {
    ...
    try {
        const persons = await product.update({ _id: id }, data)
        res.send(persons)
    } catch{
